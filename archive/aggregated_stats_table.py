...
def main():
    ...
    for season in ["20152016", "20162017", "20172018"]:
        # Get corsi data
        CORSI_QUERY = f"SELECT * FROM derived.raw_corsi_{season}"
        ...
        PLAYER_SALARY_QUERY = (
            f'SELECT "firstName", "lastName", "capHit" FROM dim.player_cap_hit_{season}'
        )
        ...
        AGGREGATED_TABLE_NAME = f"mart.aggregated_corsi_{season}"
